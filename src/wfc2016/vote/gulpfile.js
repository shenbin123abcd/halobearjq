...
gulp.task('images', function () {
    return gulp.src(['app/images/**/*.{png,gif,jpg,svg，mp3,mp4}','!app/images/src/**/*'])
        ...
        .pipe(plugins.rev())
        .pipe(gulp.dest('../../../Public/Wfc2016/vote/images'))
...
gulp.task('haloIcon', function () {
    return gulp.src('app/css/lib/ux_*/*iconfont.*')
        ...
        .pipe(gulp.dest('../../../Public/Wfc2016/vote/css'))
});
...
gulp.task('build', ['sass','images','generateDistVersion','haloIcon'], function () {
    ...
    return gulp.src(['app/*.html'])
        ...
        .pipe(plugins.cdnizer({
            // defaultCDNBase: "/Public/Wfc2016/vote",
            defaultCDNBase: "http://7ktq5x.com1.z0.glb.clouddn.com/Wfc2016/vote",
            allowRev: true,
            allowMin: true,
            matchers: [
                // /(attr\([\s\S]*src\s*:[\s\S]*?["'`])(.+?)(["'`].*?)/gi,
                /(\.attr\(\{[\s\S]*?["'`])(.+?)(["'`][\s\S]*?\}\))/gi,
            ],
            files: [
                'images/**/*',
        ...
        .pipe(plugins.uglify())
        .pipe(plugins.rev())
        .pipe(gulp.dest('../../../Public/Wfc2016/vote'))
        ...
        .pipe(gulp.dest('../../../Modules/Wfc2016/Tpl/Vote'))
});
    ...
    return gulp
        ...
        .pipe(plugins.cdnizer({
            ...
            files: [
                ...
                {
                    file: 'js/**/*.js',
                    cdn: "http://"+devip()[0]+":9000/tmp/js/${ filename }"
                },
