app.vote=(function(){
    ...
    var VOTE = {
        ...
        init: function(){
            ...
            $('body').on('click', '[vote-bt]', function(event) {
                event.preventDefault();
                _this.obj = $(this);
                _this.data.id = $(this).data('id');
                _this.data.companyName = $(this).data('companyName');
                // var obj = $('[vote-num][case="case-'+_this.data.id+'"]');
                // obj.each(function() {
        ...
        action: function(){
            ...
            if (hb.location.url('?halo') !== 'test2015') {
                if(new Date('2016/07/19 19:50:00').getTime() > new Date().getTime()){
                    hb.lib.weui.confirm({
                        content:'<div style="text-align: center;">投票将于07月19日 20:00开始，<br>敬请期待</div>',
                        rightBtn:'报名参赛'
                    }).then(function () {
                        window.location.href='/supplier/apply';
                    });
                    return false;
                }else if (new Date('2016/07/21 20:00:00').getTime() < new Date().getTime()) {
                    hb.lib.weui.alert('投票已结束');
                    return false;
                }
            }
            var works_id = localStorage.getItem('supplier_vote_id');
            ...
            if ($.type(this.works_parse[this.data.id]) !== 'undefined' && this.works_parse[this.data.id] === day) {
                hb.lib.weui.alert('每天只能为每个服务商投票一次哦！');
                return;
            }
        ...
        wechat: function(){
            if (!hb.Cookies.get('wfc2016_vote_id')) {
                window.location.href = '/supplier/wechat?url=' + this.url;
                return false;
            }
            this.data.type = 1;
            this.submit();
            ...
            $.ajax({
                ...
                success: function(res){
                    if (res.status == 1) {
                        ...
                        localStorage.setItem('supplier_vote_id',JSON.stringify(_this.works_parse));
                        ...
                        // 微信未授权
                        window.location.href = '/supplier/wechat?url=' + _this.url;
                    }else{
                        _this.obj.text('投票');
                        hb.lib.weui.alert(res.info);
                    }
